...
export function SimpleBarChart({ data, height = 100, color = "primary", className }: SimpleChartProps) {
...
export function SimpleLineChart({ data, height = 100, color = "primary", className }: SimpleChartProps) {
    const max = Math.max(...data);
        ...
        <div className={cn("space-y-1", className)}>
            {(label || showValue) && (
                <div className="flex items-center justify-between text-xs">
                    ...
                    {showValue && <span className="font-bold">{value.toLocaleString()}</span>}
                </div>
            )}

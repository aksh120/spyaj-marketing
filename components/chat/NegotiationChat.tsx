...
export default function NegotiationChat({
  ...
  const handleSendOffer = () => {
    ...
    setNewOffer({
      amount: Math.floor(newOffer.amount * 0.95),
      quantity: newOffer.quantity,
      message: "",
    });
              ...
              <div className="p-4 border-t border-gray-200 dark:border-zinc-700 bg-white dark:bg-zinc-900">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    onChange={(e) =>
                      setNewOffer({ ...newOffer, message: e.target.value })
                    }
                ...
                <div className="mt-3 p-2 bg-muted/50 rounded-lg">
                  <p className="text-xs text-muted-foreground text-center">
                    ...
                    <span className="font-bold text-foreground">
                      {currency}
                      {(newOffer.amount * newOffer.quantity).toLocaleString()}
                    </span>{" "}
                    for {newOffer.quantity} units
                  </p>
                </div>
              </div>
            </motion.div>

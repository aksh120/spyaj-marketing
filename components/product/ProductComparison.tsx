...
export function CompareButton({ product, isInCompare, onAdd, onRemove, className }: CompareButtonProps) {
    return (
        <motion.button
...
export function CompareBar({ compareList, onRemove, onClear }: CompareBarProps) {
            ...
            >
                <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="font-bold text-sm">Compare ({compareList.length}/4)</span>
                    </div>
...
function CompareModal({ products, onClose }: { products: CompareProduct[]; onClose: () => void }) {
    return (
        <>
            <motion.div
                initial={{ opacity: 0 }}
                    ...
                    <table className="w-full min-w-[600px]">
                        <thead>
                            <tr className="border-b border-border">
                                <th className="p-4 text-left text-sm font-bold text-muted-foreground w-32">Feature</th>
                                {products.map((product) => (
                                    <th key={product.id} className="p-4 text-center">
                                        ...
                                        >
                                            ...
                                            <p className="font-bold text-sm truncate max-w-[150px] mx-auto">{product.name}</p>
                                        </Link>
                                    </th>
                                ))}
                            </tr>
                        ...
                        <tbody>
                            ...
                            <tr className="border-b border-border">
                                ...
                                {products.map((product) => (
                                    <td key={product.id} className="p-4 text-center text-sm font-bold">
                                        ⭐ {product.rating}
                                    </td>
                                ))}
                            </tr>
                            ...
                            <tr className="border-b border-border">
                                <td className="p-4 text-sm font-medium text-muted-foreground">MOQ</td>
                                {products.map((product) => (
                                    <td key={product.id} className="p-4 text-center text-sm">

...
import {
  HeartPulse, Shirt, Beaker, Settings, Building2, Cpu, Stethoscope,
  Gift, Package, Sprout, Home as HomeIcon, Gem, Factory, Cog,
  ShieldCheck, Globe, Headphones, CreditCard, ChevronRight, ChevronLeft,
  Flame, Zap, Star, Heart, ShoppingBag, FileQuestion, Lock, TrendingUp,
  Sparkles, ArrowRight
} from "lucide-react";
...
const sideCategories = [
  ...
  { name: "Construction & Real Estate", icon: <Building2 className="w-4 h-4" /> },
...
const features = [
  { text: "Worry-Free Shopping", icon: <ShieldCheck className="w-5 h-5 md:w-6 md:h-6 text-yellow-500" />, desc: "100% Protection" },
  { text: "Worldwide Delivery", icon: <Globe className="w-5 h-5 md:w-6 md:h-6 text-yellow-500" />, desc: "200+ Countries" },
  { text: "24/7 Customer Service", icon: <Headphones className="w-5 h-5 md:w-6 md:h-6 text-yellow-500" />, desc: "Always Available" },
...
const categories = [
  { name: "Electronics", icon: <Cpu className="w-5 h-5 md:w-6 md:h-6 text-blue-500" />, color: "from-blue-500/20 to-blue-600/10", count: "2.5K+" },
  { name: "Fashion", icon: <Shirt className="w-5 h-5 md:w-6 md:h-6 text-pink-500" />, color: "from-pink-500/20 to-pink-600/10", count: "5K+" },
  { name: "Industrial", icon: <Factory className="w-5 h-5 md:w-6 md:h-6 text-gray-500" />, color: "from-gray-500/20 to-gray-600/10", count: "1.8K+" },
  { name: "Health", icon: <HeartPulse className="w-5 h-5 md:w-6 md:h-6 text-red-500" />, color: "from-red-500/20 to-red-600/10", count: "3.2K+" },
  { name: "Agriculture", icon: <Sprout className="w-5 h-5 md:w-6 md:h-6 text-green-500" />, color: "from-green-500/20 to-green-600/10", count: "1.5K+" },
];
  ...
  return (
    <main className="min-h-screen pt-[80px] md:pt-[100px] flex flex-col bg-background overflow-hidden">
      <div className="max-w-[1400px] mx-auto w-full px-4 md:px-6 flex flex-col gap-4 md:gap-6">
        ...
        >
          ...
          <div className="absolute inset-0 flex items-center justify-between px-4 md:px-12">
            ...
            <motion.div
              ...
              whileTap={{ scale: 0.95 }}
            >
              <Link href="/marketplace" className="hidden md:flex items-center gap-2 bg-white/20 backdrop-blur-sm text-white px-6 py-2 rounded-full font-bold hover:bg-white/30 transition-all border border-white/30">
        ...
        <div ref={heroRef} className="flex flex-col lg:flex-row gap-4 md:gap-6 lg:h-[500px]">
          <motion.div
          ...
          {features.map((feature, i) => (
            ...
            >
              ...
              <div>
                ...
                <span className="text-[10px] md:text-xs text-muted-foreground">{feature.desc}</span>
              </div>
            </motion.div>
          ))}
        </motion.div>
        ...
        <AnimatedSection className="bg-gradient-to-r from-orange-50 to-orange-100 dark:from-orange-950/20 dark:to-orange-900/20 border-2 border-orange-200 dark:border-orange-800 rounded-xl md:rounded-2xl p-4 md:p-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 md:gap-0 mb-4 md:mb-6">
            <div className="flex flex-wrap items-center gap-2 md:gap-3">
              ...
              <span className="text-xs md:text-sm text-muted-foreground bg-orange-200/50 dark:bg-orange-800/30 px-2 py-0.5 rounded-full">Free Shipping | Save Up to ₹5000!</span>
            </div>
            <Link href="/marketplace" className="text-xs md:text-sm font-bold text-primary hover:underline flex items-center gap-1 group">
        ...
        <AnimatedSection className="bg-gradient-to-r from-red-50 to-pink-100 dark:from-red-950/20 dark:to-pink-900/20 border-2 border-red-200 dark:border-red-800 rounded-xl md:rounded-2xl p-4 md:p-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 md:gap-0 mb-4 md:mb-6">
            <div className="flex flex-wrap items-center gap-2 md:gap-4">
              ...
              <h2 className="text-lg md:text-2xl font-black text-red-600">Flash Deals</h2>
          ...
          >
            {flashDeals.map((product) => (
              <ProductCard key={product.id} product={product} showDiscount showStock />
            ))}
          </motion.div>
        </AnimatedSection>
        ...
        <AnimatedSection className="bg-card border-2 border-border rounded-xl md:rounded-2xl p-4 md:p-6 shadow-md">
          <div className="flex items-center justify-between mb-4 md:mb-6">
            <div className="flex items-center gap-2 md:gap-3">
              ...
              <h2 className="text-lg md:text-2xl font-black">Featured Products</h2>
            </div>
            <Link href="/marketplace" className="text-xs md:text-sm font-bold text-primary hover:underline flex items-center gap-1 group">
              View All <ChevronRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
            </Link>
...
function ProductCard({ product, showDiscount, showStock }: { product: any, showDiscount?: boolean, showStock?: boolean }) {
  return (
    <motion.div
      variants={staggerItem}
      whileHover={{ y: -8, boxShadow: "0 20px 40px rgba(0,0,0,0.15)" }}
      className="bg-card border-2 border-border rounded-xl p-3 hover:border-primary/50 transition-all group cursor-pointer"
    >
      <div className="aspect-square bg-gradient-to-br from-primary/5 to-primary/10 rounded-lg mb-3 flex items-center justify-center relative overflow-hidden">
        ...
        {showDiscount && (
          <motion.div
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
            className="absolute top-2 left-2 bg-red-600 text-white text-[10px] md:text-xs font-bold px-1.5 md:px-2 py-0.5 md:py-1 rounded"
          >
            {product.discount}
          </motion.div>
        )}
        {showStock && product.stock && (
          <div className="absolute bottom-2 left-2 right-2">
            ...
            <span className="text-[9px] text-muted-foreground mt-0.5 block">{product.stock} left</span>
          </div>
        )}
        <motion.button
          whileHover={{ scale: 1.1 }}
          whileTap={{ scale: 0.9 }}
          className="absolute top-2 right-2 p-1.5 bg-white/80 dark:bg-black/50 rounded-full opacity-0 group-hover:opacity-100 transition-opacity"
        >
          <Heart className="w-3 h-3 text-gray-500" />
        </motion.button>
      </div>
      <h4 className="text-xs font-semibold mb-1 line-clamp-2 group-hover:text-primary transition-colors">{product.name}</h4>
      {product.rating && (
        <div className="flex items-center gap-1 mb-1">
          <Star className="w-3 h-3 text-yellow-500 fill-yellow-500" />
          <span className="text-[10px] text-muted-foreground">{product.rating} ({product.sold || product.stock} sold)</span>
        </div>
      )}
      <div className="flex items-baseline gap-1 md:gap-2">
        <span className="text-sm font-bold text-primary">{product.price}</span>
        {product.oldPrice && (
          <span className="text-[10px] md:text-xs text-muted-foreground line-through">{product.oldPrice}</span>
        )}
      </div>
    </motion.div>
  );
    ...
    >
      ...
      <div className="p-3 md:p-4">
        <div className="flex items-center gap-1.5 md:gap-2 mb-1.5 md:mb-2">
          ...
          />
          <span className="text-[10px] md:text-xs font-bold text-blue-500 uppercase">{product.seller}</span>
          {product.verified && <ShieldCheck className="w-3 h-3 text-green-500" />}
        </div>
        <h4 className="text-xs md:text-base font-bold mb-1.5 md:mb-2 group-hover:text-primary transition-colors line-clamp-2">{product.name}</h4>
        <div className="flex items-center gap-1 mb-2">
          <Star className="w-3 h-3 text-yellow-500 fill-yellow-500" />
          <span className="text-[10px] text-muted-foreground">{product.rating} • {product.orders} orders</span>
        </div>
        <p className="text-sm md:text-xl font-bold text-primary">{product.price}</p>
      </div>
    </motion.div>
  );
}
...
function CategoryCarousel({ category }: { category: any }) {
  ...
  return (
    <AnimatedSection className="bg-card border-2 border-border rounded-xl md:rounded-2xl p-4 md:p-6 shadow-md">
      <div className="flex items-center justify-between mb-4 md:mb-6">
        <div className="flex items-center gap-2 md:gap-3">
          ...
          <div>
            ...
            <span className="text-[10px] md:text-xs text-muted-foreground">{category.count} products</span>
          </div>
        </div>
      ...
      <div ref={scrollRef} className="flex gap-3 md:gap-4 overflow-x-auto scrollbar-hide scroll-smooth -mx-4 px-4 md:mx-0 md:px-0 pb-2">
        {products.map((product, idx) => (
          <HorizontalProductCard key={product.id} product={product} index={idx} />
        ))}
      </div>
    </AnimatedSection>
  );
...
function HorizontalProductCard({ product, index }: { product: any, index: number }) {
  return (
    <motion.div
      initial={{ opacity: 0, x: 20 }}
    ...
    >
      ...
      <div className="flex-1 flex flex-col justify-between">
        <div>
          <h4 className="text-xs md:text-sm font-bold mb-0.5 md:mb-1 line-clamp-2 group-hover:text-primary transition-colors">{product.name}</h4>
          <p className="text-[10px] md:text-xs text-muted-foreground mb-1">{product.seller}</p>
